## Experimental multi-rotator embedding.

### A mix of:
   ####  Givens Rotation 1/3
   ####  Householder (or quaternion) Rotation 1/3
...
--##### quaternion in place of householder rotation
      ...
      class EfficientBlendedRotaryEmbedding(nn.Module):
          ...
          def quaternion_rotation(self, x, theta, u, v):
              ...
              # Quaternion q = cos(theta/2) + sin(theta/2) * (u x v)
              q = cos_theta + sin_theta * torch.cross(u, v)
              
              # Quaternion conjugate q* = cos(theta/2) - sin(theta/2) * (u x v)
              q_conjugate = cos_theta - sin_theta * torch.cross(u, v)
              
              # Apply quaternion rotation
              x_rot = torch.cross(q, torch.cross(q_conjugate, x) + (torch.dot(q, x) * torch.eye(x.size(-1)).to(x.device)))
          ...
          def blended_rotation_matrix(self, dims, i, j, theta):
              ...
              # Quaternion rotation with unit vectors
...
--##### Toggelable i.e. rotation_type='givens'
